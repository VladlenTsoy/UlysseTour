            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form class="was-validated" method="POST" action="/admin/language">
                            {!! csrf_field() !!}
                                ...
                                @foreach($languages->data as $key => $item)
                                    ...
                                    @elseif($key === 'pluses')
                                        @foreach($item as $key_pluses => $val)
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="plus_title_{{$key_pluses}}">Название плюса #{{$key_pluses+1}}</label>
                                                    <input type="text" class="form-control" id="plus_title_{{$key_pluses}}"
                                                           name="pluses_title[]" required value="{{$val->title}}">
                                                </div>
                                                <div class="form-group">
                                                    <label for="plus_desc_{{$key_pluses}}">Название плюса #{{$key_pluses+1}}</label>
                                                    <textarea type="text" class="form-control" id="plus_desc_{{$key_pluses}}"
                                                           name="pluses_description[]" required>{{$val->description}}</textarea>
                                                </div>
                                            </div>
                                        @endforeach
                                    @endif
